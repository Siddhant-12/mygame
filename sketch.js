...
function setup() 
 {
  ...
  start=createSprite(20,20,10,10)
  start.addImage(startimg)
  start.scale=1

  pause=createSprite(20,40,10,10)
  pause.addImage(pauseimg)
  pause.scale=1


  instructions=createSprite(20,60,10,10)
...
function spawnbottles(){
  ...
  if(frameCount % 80 === 0){
    ...
    bottle.x=randx
    bottle.y=randy
    //bottle.addImage("droplet",dropletImg);
    var rand = Math.round(random(1,3))
    switch(rand){
 case 1: bottle.addImage(bottle1_Img);
 break;

 case 2: bottle.addImage(bottle2_Img);
 break;
